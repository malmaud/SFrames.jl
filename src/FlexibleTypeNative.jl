...
function FlexibleType(x::AbstractString)
    icxx"return flexible_type($(cstring(x)));" |> FlexibleType
end

function FlexibleType(c_flex_type::Cxx.CppValue)
    bytes = c_flex_type.data
    # todo make a generated function for this
    data = reinterpret(Int, UInt64(bytes[1]) | UInt64(bytes[2])<<8 | UInt64(bytes[3])<<16 | UInt64(bytes[4])<<24) | UInt64(bytes[5])<<32 | UInt64(bytes[6])<<40 | UInt64(bytes[7])<<48 | UInt64(bytes[8])<<56
    typ = bytes[13]
    FlexibleType(data, typ)
end
...
function Base.bytestring(f::FlexibleType)
    ref = icxx"((flexible_type*)$(pointer_from_objref(f)))->get<flex_string>();"
    icxx"$ref.size();" |> Int
end

r=FlexibleType("jon")

ptr=icxx"(flexible_type*)($(pointer_from_objref(r)));"
t=icxx"*$ptr;"
icxx"((flex_string)$t).c_str();"
